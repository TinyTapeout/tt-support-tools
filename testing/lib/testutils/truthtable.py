...
class TruthTable:
    ...
    async def testAll(self, i_bus, o_bus, logger=None):
        for i in range(len(self)):
            ...
            for idx in range(8):
                logger.debug(
                    f"{i}-{idx}: i_bus[{idx}] == {i_bus[idx]} == {self[i].state[idx]}"
                )

            expectedResult = self[i].result
            if logger is not None:
                logger.info(
                    f"State {i}, setting input to {self[i].state}, expecting {expectedResult} (got {o_bus.value})"
                )
...
class TestTableEntryParser:
    ...
    def __init__(self, logger):
        ...
        self.validOutputValues = (
            r"(\s*[01xX-]\s*){8}"  # 0/1 hard-coded,x or - don't care
        )
...
class TestTableParser:
    ...
    def addStep(self, inputValue: str, outputValue: str = ""):
        ...
        if inputBits.find("c") >= 0:
            ...
            self.log.debug(
                f"Have clocking in inputs {inputBits}, adding setup step {setupInputs}"
            )
        ...
        self.log.info(
            f"Mapped {inputBits} -> {outputBits} ({inputBinVal} -> {outputBinVal})"
        )

        self.truthTable.addMapping(
            OneToOneTruthMapping(inputBinVal.binstr, outputBinVal.binstr)
        )
...
class MarkdownTestTableParser(TestTableParser):
    ...
    def __init__(self, columnSeparatorRegex: str = r"\|", logger=None):
        ...
        self.headerSepRe = re.compile(
            tableHeaderSepTpl.substitute(sep=self.columnSeperatorRe, eol=eol), re.M
        )

        # each content line thereafter
        # | CONTENT | CONTENT |
        contentRowTpl = StringTemplate(r"^\s*(($sep\s*[^$sep\r\n]+){1,})$sep$eol")
        self.contentRowRe = re.compile(
            contentRowTpl.substitute(sep=self.columnSeperatorRe, eol=eol), re.M
        )
    ...
    def parseMarkdownTable(self, contents: str):
        ...
        if headerSepSearch is not None:
            ...
            self.log.debug(
                f"Dumping header to {headerEndPos}\n{contents[:headerEndPos]}"
            )
